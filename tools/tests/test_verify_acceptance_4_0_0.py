"""Unit tests for the 4.0.0 acceptance verifier (W-14: no junk evidence, hard gate)."""
...
SCRIPT = Path(__file__).resolve().parents[1] / "verify_acceptance_4_0_0.py"
SPEC = importlib.util.spec_from_file_location("verify_acceptance_4_0_0_test", SCRIPT)
...
class AcceptanceVerifierTests(unittest.TestCase):
    ...
    def verify(self, phase: str = "prepublish", allow_pending: bool = True) -> int:
        return qa.verify(
            argparse.Namespace(
                manifest=self.manifest, phase=phase, allow_pending=allow_pending
            )
        )
    ...
    def evidence_case(self, name: str, payload: bytes, kind: str) -> dict:
    ...
    def test_waived_case_without_reason_is_rejected(self) -> None:
        ...
        self.assertIn("waiver object", " ".join(qa.validate_manifest(data)))
        data["cases"][0]["waiver"] = {"approver": "R. Corbin", "reason": "   "}
        self.assertIn("waiver.reason", " ".join(qa.validate_manifest(data)))
