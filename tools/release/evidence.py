#!/usr/bin/env python3
"""Build reproducible release evidence for a Shadowfetch Linux release.

ONE implementation for every release; the version identity is DATA in
tools/release/versions/<version>.toml.

The evidence this writes is what a reader is asked to trust about the artifact,
so the programs that produce it -- unsquashfs here, and mount/umount/findmnt
inside iso_gate.mounted_iso -- are resolved to trusted absolute paths rather
than found on PATH.
"""
...
import gate
import iso_gate
from iso_gate import mounted_iso


# Assigned in main() from the version data. They stay module-level because every
...
REQUIRED_PROGRAMS = (
    ("findmnt", gate.ROLE_SECURITY),
    ("mount", gate.ROLE_SECURITY),
    ("sudo", gate.ROLE_SECURITY),
    ("umount", gate.ROLE_SECURITY),
    ("unsquashfs", gate.ROLE_SECURITY),
)
...
def build_iso_inventory(
    ...
    with mounted_iso(iso_path) as mountpoint:
        ...
        with tempfile.TemporaryDirectory(prefix="shadowfetch-evidence-inventory-") as temporary:
            ...
            subprocess.run(
                iso_gate.program("unsquashfs").argv(
                    "-processors", "2", "-no-progress", "-no-xattrs",
...
def main() -> int:
    ...
    parser.add_argument("--root", type=Path, default=gate.ROOT)
    ...
    for resolved in resolver.require(REQUIRED_PROGRAMS):
        iso_gate.PROGRAMS[resolved.name] = resolved
    ...
    packages_index = root / f"repo/dists/{release.codename}/main/binary-amd64/Packages"
    sources_index = root / f"repo/dists/{release.codename}/main/source/Sources"
    iso_path = root / release.iso_name
