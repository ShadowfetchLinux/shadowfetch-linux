...
for tool in stress-ng podman shadowfetch-missions shadowfetch-grok-bot ffmpeg ffprobe; do
...
setsid "${AS_USER[@]}" python3 "$helper_dir/container_stress.py" --duration "$duration" --image "$image_id" --run-id "$run_id" > "$out/container-loop.log" 2>&1 &
container_pid=$!

setsid "${AS_USER[@]}" python3 "$helper_dir/mission_stress.py" --duration "$duration" --run-id "$run_id" --output "$qa_home/.local/state/shadowfetch/qa-stress-$run_id" > "$out/mission-loop.log" 2>&1 &
mission_pid=$!
...
load_elapsed=$((end_epoch - load_started))
printf 'end=%s\nend_epoch=%s\nelapsed_seconds=%s\n' \
    "$end_iso" "$end_epoch" "$elapsed" >> "$out/timing.txt"

journalctl -b --since "@$start_epoch" --no-pager > "$out/journal-since-start.log" || record_failure "system journal unavailable"
grep -Eai "Failed to start |Failed with result|Main process exited, code=|core-dump" "$out/journal-since-start.log" > "$out/service-faults.txt" || true
...
as_user podman info --format json > "$out/podman-info-after.json" || record_failure "rootless podman info failed after load"
as_user podman image inspect "$image" > "$out/container-image-inspect.json"
...
for rc in "$pull_rc" "$plans_rc" "$stress_rc" "$container_rc" "$probe_rc" "$mission_rc"; do
    if (( rc != 0 )); then
        failures=$((failures + 1))
    fi
done
